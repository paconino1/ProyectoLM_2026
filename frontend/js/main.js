...
document.addEventListener('DOMContentLoaded', () => {
    ...
    categoryCards.forEach(card => {
        card.addEventListener('click', (e) => {
            const platform = e.currentTarget.textContent.match(/PC|PlayStation|Xbox|Nintendo/);
            if (platform) {
...
async function loadFeaturedGames() {
    ...
    try {
        ...
        if (!games || games.length === 0) {
            container.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #999;">No hay juegos disponibles.</p>';
            return;
        }
        ...
        container.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #e74c3c;">Error al cargar juegos.</p>';
    }
}
...
function showGameModal(game) {
    ...
    document.getElementById('modal-game-image').src = game.imagen || 'assets/default.jpg';
    document.getElementById('modal-game-name').textContent = game.nombre;
    document.getElementById('modal-game-description').textContent = game.descripcion || 'Sin descripción disponible';
    document.getElementById('modal-game-platform').textContent = game.plataforma;
    document.getElementById('modal-game-stock').textContent = game.stock > 0 ? 'En stock' : 'Agotado';
    
    const discount = game.descuento || 0;
    if (discount > 0) {
        document.getElementById('modal-original-price').textContent = `€${game.precio.toFixed(2)}`;
    } else {
        document.getElementById('modal-original-price').textContent = '';
    }
    document.getElementById('modal-discount-price').textContent = `€${(game.precioConDescuento || game.precio).toFixed(2)}`;
