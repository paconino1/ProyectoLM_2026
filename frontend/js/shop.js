...
function displayGames(games) {
  ...
  if (games.length === 0) {
    grid.innerHTML =
      '<p style="grid-column: 1/-1; text-align: center; color: #999;">No se encontraron resultados.</p>';
    return;
  }
...
function applyFilters() {
  ...
  if (sortOrder === "price-asc") {
    filtered.sort(
      (a, b) =>
        (a.precioConDescuento || a.precio) - (b.precioConDescuento || b.precio),
    );
  } else if (sortOrder === "price-desc") {
    filtered.sort(
      (a, b) =>
        (b.precioConDescuento || b.precio) - (a.precioConDescuento || a.precio),
    );
